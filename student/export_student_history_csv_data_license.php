<?php
	include "../config/config.php";
		...
		//print_r($reportreq);die();
		$report_type   = 'student_history' ;
		$report_type1  = 'student_history_license' ;
		sort($student_ids);
		...
		$check = "SELECT id FROM export_queue WHERE user_id = '$user_id' AND status = 'pending' AND csv_type = '$report_type' AND is_single = '0' LIMIT 1";
		...
		if ($request_hash[0] == $report_hash) {
			...
			echo json_encode([
					"Success" => "200",
					"msg" => "<h4><b>" .$display_name . "</b> is already included in a <b>Typio</b> report today. Please check the <a href='" . ADMIN_URL . "student/student-reports.php' class='' id='list_lnk_mystu'>Students Reports</a> page.</h4>"
				]);
		} else {
			// Insert new request
			...
			$request = "UPDATE report_requests SET request_count = request_count + 1
                   ...
                   AND csv_type = '$report_type' AND is_single = '0'";
			...
			$query = "INSERT INTO export_queue (user_id, user_email, status , csv_type , student_data , stuadm ,dwn_lnk_id, is_single) VALUES ('$user_id', '$user_email', '$status' ,'$report_type' , '$student_ids_json' , '$stuadm' ,
					  '$inserted_id' , '0' )";
					  //print_r($query);die();
			...
			if ($query_result) {
				echo json_encode([
					"Success" => "200",
					"msg" => "<h4>Generating <b>Typio</b> report. Please check the <a href='".ADMIN_URL."student/student-reports.php' class='' id='list_lnk_mystu' >Students Reports</a> page to access your report.</h4>"
				]);
			} else {
				echo json_encode([
					"Success" => "500",
