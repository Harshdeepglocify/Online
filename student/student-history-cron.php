...
if (!$con) {
	...
	$check = "SELECT user_id, user_email ,student_data ,stuadm , dwn_lnk_id , is_single FROM export_queue WHERE status = 'pending' AND csv_type = 'student_history' LIMIT 100 ";
...
if ($result && mysqli_num_rows($result) > 0) {
    while ($row = mysqli_fetch_assoc($result)) {
        $existing_user_id[] = array("user" => $row['user_id'] , "email" => $row['user_email'] , "studentid" => $row['student_data'] , "stuadm" => $row['stuadm'], 'download_id' => $row['dwn_lnk_id'] ,'is_teacher' => $row['is_single']);
		...
		/*$filename = "Accessibyte_Typio_History_" . time().".csv";
		$csvFile  = $csvFolder . "/" . $filename;
		$f = fopen($csvFile, "w");*/  
		/*$filename_teacher = "Accessibyte_Typio_History_" . time() . ".csv";
		...
		$filename_license = "Student_Typio_History_" . time() . ".csv";
	  ...
	  $j = 0;
		foreach($existing_user_id as $alldata){
			$unique = uniqid();
			if($alldata['stuadm'] == "1" || $alldata['stuadm'] == 1){
				$filename = "Student_Typio_History_" . $unique. $j . ".csv";
				//$filename = "Student_Typio_History_" . time() . ".csv";
				$csvFile  = $csvFolder . "/" . $filename;
				$f = fopen($csvFile, "w");
				$fields = array('Student Username','Date','Lesson','WPM','Accuracy', 'Errors');	
			
			}else{
				$filename = "Accessibyte_Typio_History_" . $unique. $j . ".csv";
				//$filename = "Accessibyte_Typio_History_" . time() . ".csv";
				$csvFile  = $csvFolder . "/" . $filename;
				$f = fopen($csvFile, "w");
				$fields = array('Date','Student Username','Lesson','WPM','Accuracy', 'Errors');
	...
	$update_sql = "UPDATE report_downloads 
		...
		if (mysqli_query($con, $update_sql)) {
		   ...
		   $delete = "DELETE FROM export_queue WHERE user_id = '".$alldata['user']."' AND user_email = '".$alldata['email']."'  AND csv_type = 'student_history' AND is_single = '".$alldata['is_teacher']."' ";
