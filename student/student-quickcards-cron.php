...
if (!$con) {
	...
	$check = "SELECT user_id, user_email ,student_data , dwn_lnk_id,is_single
          FROM export_queue 
          WHERE status = 'pending' AND csv_type = 'quick_cards'  LIMIT 100 ";
...
if ($result && mysqli_num_rows($result) > 0) {
    while ($row = mysqli_fetch_assoc($result)) {
		...
		$unique = uniqid();
		$filename 	= "Accessibyte_Quick_Cards_History_" . $unique.".csv";
...
foreach($existing_user_id as $alldata){
	...
	$update_sql = "UPDATE report_downloads 
		...
		if (mysqli_query($con, $update_sql)) {
			$delete = "DELETE FROM export_queue WHERE user_id = '".$alldata['user']."' AND user_email = '".$alldata['email']."'  AND csv_type = 'quick_cards' AND is_single = '".$alldata['is_teacher']."' ";
