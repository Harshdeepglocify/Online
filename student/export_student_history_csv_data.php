<?php
	include "../config/config.php";
		...
		$limit       	= 5;	
		$report_type  	= 'student_history';
		$todaydate 		= date('Y-m-d');
		//$todaydate 		= '2025-11-07';
		...
		if (is_array($student_ids) && count($student_ids) > 1) {
			...
			/*if ($reportreq === $report_hashs1[0]) {
    
			echo json_encode([
					"Success" => "200",
					"msg" => "<h4>You have already requested these <b>Students</b> in an <b>Activity</b> report. You won’t be able to generate a new one. Please check the <a href='" . ADMIN_URL . "student/student-reports.php'      class='history_hide' id='list_lnk_mystu'>Student Reports</a> page.</h4>"	]);
				exit();
			...
			foreach ($report_hashs as $subArray) {
				if (is_array($subArray)) {
					// handle sub-array
					if (in_array($student_ids[0], $subArray)) {
						$found = true;
						break;
					}
				} else {
					// handle single-level array
					if ($subArray == $student_ids[0]) {
						$found = true;
						break;
					}
				}
			...
			/*if($found) {
				...
				echo json_encode([
					"Success" => "200",
					"msg" => "<h4><b>" . $reportreq . "</b> is already included in a <b>Activity</b> report. 
							  You won’t be able to generate a new one. Please check the 
							  <a href='" . ADMIN_URL . "student/student-reports.php' 
							  class='history_hide' id='list_lnk_mystu'>Students Reports</a> page.</h4>"
				]);
				exit();
		...
		if ($request_hash[0] == $report_hash) {
			...
			echo json_encode([
					"Success" => "200",
					"msg" => "<h4><b>" .$display_name . "</b> is already included in a <b>Typio</b> report today. Please check the <a href='" . ADMIN_URL . "student/student-reports.php' class='history_hide' id='list_lnk_mystu'>Students Reports</a> page.</h4>"
				]);
		} else {
			// Insert new request
			$user_id    	= $_SESSION['User']['id'];
			...
			$sql_downloads 	= "INSERT INTO report_downloads (user_id, user_email, file_status , report_hash, report_type,is_single, created_at) VALUES ('$user_id', '$user_email', 'Processing' , '$reportreq' ,
			'$report_type' , '$teacher_data' , NOW() )";
			...
			$query = "INSERT INTO export_queue (user_id, user_email, status , csv_type , student_data , stuadm ,dwn_lnk_id, is_single) VALUES ('$user_id', '$user_email', '$status' ,'$report_type' , '$student_ids_json' , '$stuadm' , '$inserted_id' ,'$teacher_data' )";
			...
			if ($query_result) {
				echo json_encode([
					...
					"msg" => "<h4>Generating <b>Typio</b> report. Please check the <a href='".ADMIN_URL."student/student-reports.php' class='history_hide' id='list_lnk_mystu' >Students Reports</a> page to access your report.</h4>"
				]);
			} else {
				echo json_encode([
					"Success" => "500",
	...
	$student_ids = isset($_POST['ids']) ? $_POST['ids'] : ''; 
	$stuadm      = isset($_POST['stuadm']) ? $_POST['stuadm'] : '0';
	if($stuadm == "1" || $stuadm == 1){
	 $fields = array('Student Username','Date','Lesson','WPM',' Accuracy', 'Errors');	
        ...
        foreach($student_ids as $studentid){
            ...
            if (mysqli_num_rows($query_result) > 0 ) {
                while ($log_typio_data_row = mysqli_fetch_assoc($query_result)) {
					...
					if($stuadm == "1" || $stuadm == 1){
                        $fields_value = array($username, $date, $title, $wpm ,$acc , $error_data);
					}else{
