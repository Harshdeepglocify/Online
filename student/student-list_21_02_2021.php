...
<html>
                                        ...
                                        <tbody> 
                                            ...
                                            if (isset($users) && !empty($users)) {
                                                foreach ($users as $user) {
                                                    echo '<tr>';
													echo '<td>' . ucfirst(base64_decode($user['firstname'])) . '</td>';
													echo '<td>'.base64_decode($user['username']).'</td>';
                                                    echo '
													<td>
  ...
  <span> <a href="' . ADMIN_URL . 'student/student-overview.php?student=' . $user['id'] . '" aria-label="Overview for ' . ucfirst(base64_decode($user['firstname'])) .' " class="accessibyte-link">Overview</a>
    ...
    <a href="' . ADMIN_URL . 'student/student-password.php?student=' . $user['id'] . '" class="accessibyte-link" aria-label="Change Password for ' . ucfirst(base64_decode($user['firstname'])) .' ">Change Password</a></span>';
                                                    echo '<span>
      <a href="javascript:void(0)" data-id="' . $user['id'] . '" data-name="' . ucfirst(base64_decode($user['firstname'])) . '(' . base64_decode($user['username']) . ')" class="accessibyte-link delete_student" aria-label="Delete ' . ucfirst(base64_decode($user['firstname'])) .' ">Delete</a>
      </span>
      </div>
      </td>';
                                                    echo '</tr>';
...
// echo "<pre>";print_r($no_of_licence);
                                if (!empty($no_of_licence) && isset($no_of_licence['no_student_use'])) {
                                    ...
                                    echo "</h3></div>";
                                    ?>                              
                                    <div class="col-md-3"><h4 class='student-custimize-text'>Seats in use :  <?php echo $used . ' of ' . $studentUsed; ?></h4></div>
                                    <div class="col-md-3"><h4 class='student-custimize-text'>Student Seats available : <?php echo $availseat; ?></h4></div>
                                    <?php
                                } else {
                                    echo "<div class='col-md-2'><h3>";
                                    if (!empty($licenseArr)) {
                                    ...
                                    if (!empty($licenseArr)) {
                                        ...
                                        $used = $totle - $left;
                                        ?>
                                        <div class="col-md-3"><h4 class='student-custimize-text'>Student seats in use :  <?php echo $used . ' of ' . $totle; ?></h4></div>
                                        <div class="col-md-3"><h4 class='student-custimize-text'>Student Seats available : <?php echo $left; ?></h4></div>
                                    <?php
                                    }
                                }
                                ?>
