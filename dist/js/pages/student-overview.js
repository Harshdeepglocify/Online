$(function () {
    ...
    $(".common-option-btn").click(function () {
        $(".common-option-section-wrap").toggle();        
    });
    /* Added BY PHP Dev 6 ON 06-09-2021 */
...
jQuery(document).ready(function ($) {
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            ...
                            if (response.status == 1) {
                                ...
                                var table = $('#students_decks_tbl').DataTable();
    ...
    $(document).on('click', '.pro-pack-save-settings-data', function () {
...
$('#student_quick_card_lesson_list_table tr:last').after(response.html);
...
$(document).on('click', '#add-student-test-btn', function () {
    ...
    var type = $('#quick_card_student_type').val();
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        if (response.status == 1) {
                            ...
                            var table = $('#student_quick_card_test_list_table').DataTable();
                            table.clear().draw();
                            var table = $('#students_test_tbl').DataTable();
                                table.ajax.reload(null, false); 
...
/* Start : Student Overview Graph Create */
   function display_all_graph(data){
    Highcharts.setOptions({
     colors: ['#ff0066', '#7cb5e9', '#f7a35c', '#90ed7d','#16E2F5']
    });
    ...
    chart: {
        ...
        spacingLeft: 0,
        spacingRight: 0,
        marginTop: 0, // Ensuring no margin at the top
        margin: [0, 0, 0, 0],
    ...
    title: {
        text: '',
        margin: 0 // No space for the title
    },
    subtitle: {
        text: '',
        margin: 0 // No space for the subtitle
    },
    xAxis: {
        categories: [''],
        title: {
...
gridLineWidth: 0
    ...
    yAxis: {
        min: 0,
        startOnTick: false, // Prevent extra space from starting on a tick
        endOnTick: false,   // Prevent extra space at the end
        title: {
            text: ''
        },
        labels: {
...
/* Start : History Graph */
 var graph_history_start_date = $('#HistoryDate1').val();

        var graph_history_end_date = $('#HistoryDate2').val();

        var student_id     = $('#graph_student_id').val();

student_overall_history_graph(student_id,graph_history_start_date,graph_history_end_date);
    function student_overall_history_graph(student_id="",graph_history_start_date="",graph_history_end_date=""){
        var response_data;
        try{
        $.ajax({
            ...
            data: {action_type: 'get_all_bar_student_history_data', start_date: graph_history_start_date, end_date: graph_history_end_date, student_id: student_id},
           ...
           //    if(response !=""){
            if(response !=null && response !="") {
                ...
                if (response.status == '200') {
                    if(response.data_available == "1"){
                        display_overall_histrory_graph(response.html);
                    }
                }
             }
              //   } 
 ...
 function display_overall_histrory_graph(data){
    Highcharts.setOptions({
     colors: ['#ff0066', '#7cb5e9', '#f7a35c', '#90ed7d','#16E2F5']
    });
 ...
 /* END : History Graph */
 function display_quick_card_history_data(){
    var qc_start_date = $("#qc-start-date").val();
    var qc_end_date   = $("#qc-end-date").val();
    var student_id = $('.qc-student-id').val();
    $('#qc-table-display-wrap').removeClass('history_hide');
    $('.quickcards_history').removeClass('history_hide');
    $.ajax({
          type: 'post',
          url: ADMIN_URL + 'config/config-student.php',
          data: {'action':'set_ajax_quick_card_history_date_field','student_id' : student_id,'is_ajax' : '1','qc_start_date':qc_start_date,'qc_end_date':qc_end_date},
          async: true,
          dataType:'json',
          success: function (response) {
           
           // console.log(response.chart_html);
            $('#qc-history-section').find('#Quickcards_general_report_tabel tbody').html(response.chart_html);
			$("h3.dashboard-h3.cst_lesson strong").text(response.totalCount+' Lessons complete');
            $('#qc-history-section .history_hide').removeClass('history_hide');
            $('#qc-history-section .no-history-msg-wrap').hide();
            var qcharts = $('#Quickcards_general_report').highcharts();
            var qoptions = qcharts.options;
 ...
 function display_quick_card_week_history_data(){
    var qc_start_date 	= $("#qc-start_week_cst").val();
    var qc_end_date   	= $("#qc-end_week_cst").val();
    var student_id 		= $('#qc-student-id_cst').val();
    $('#qc-table-display-wrap').removeClass('history_hide');
    $('.quickcards_history').removeClass('history_hide');
    $.ajax({
          type: 'post',
 ...
 $(document).on('click', '#new-option-btn', function () {
        $("#new-option-section-wrap").toggle();
        $(this).toggleClass('import-menu-display');
 });
$(document).on('click', '#new-option-btn-card', function () {
console.log('fbowefwef');
        $("#new-option-section-wrap-card").toggle();
        $(this).toggleClass('import-menu-display');
 });
$(document).on('click', '#new-option-btnBRL', function () {
        $("#new-option-section-wrapBRL").toggle();
        $(this).toggleClass('import-menu-display');
 });
 $("#new-option-btn-testBRL").click(function () {
    $("#new-option-section-wrap-testBRL").toggle();    
});
...
setTimeout(function(){ 
    ...
    //$('#piechartOverview2 .highcharts-legend-item tspan').click(function () {
    $(document).on("click","#piechartOverview2 .highcharts-legend-item tspan",function() {    
        var type=$(this).html();
        var graph_start_date = $('#HistoryDate1').val();

        var graph_end_date = $('#HistoryDate2').val();
        ...
        else if(type == "Quick Cards"){
            type ="Quick-Cards-OL"; 
        }
        else if(type == "Arcade"){
            type ="Arcade-OL"; 
        }
         else if(type == "Braillio"){
            type ="Typio-BRL";
        }
        
        $(".overview-history-graph-tbl table ."+type).toggleClass("hide-tbl-record");
        var total_history_rec = $(".overview-history-graph-tbl").find("tbody").find("tr:not(.hide-tbl-record)").length;
        $("#TotalRow").html(total_history_rec);
       
         var display_type ="";
        if($(".overview-history-graph-tbl table .Typio-OL").length > 0){  
        if(!$(".overview-history-graph-tbl table .Typio-OL").hasClass("hide-tbl-record")){
        ...
        if($(".overview-history-graph-tbl table .Propack").length > 0){
            if(!$(".overview-history-graph-tbl table .Propack").hasClass("hide-tbl-record")){
                if(display_type !=""){
         ...
         if($(".overview-history-graph-tbl table .Quick-Cards-OL").length > 0){  
            if(!$(".overview-history-graph-tbl table .Quick-Cards-OL").hasClass("hide-tbl-record")){
                if(display_type !=""){
         ...
         if($(".overview-history-graph-tbl table .Arcade-OL").length > 0){ 
            if(!$(".overview-history-graph-tbl table .Arcade-OL").hasClass("hide-tbl-record")){
                if(display_type !=""){
...
if($(".overview-graph-tbl table .Typio-BRL").length > 0){  
        if(!$(".overview-graph-tbl table .Typio-BRL").hasClass("hide-tbl-record")){
             
                if(display_type !=""){
                    display_type += ","+"Typio-BRL";
                }else{
                    display_type ="Typio-BRL";
                }
             }
         ...
         $.ajax({
           ...
           // data: {action_type: 'get_all_bar_history_hours'},
            ...
            success: function (response) {
                var response = $.parseJSON(response);
                if (response.status == '200') {
                     $(".overview-history-graph-total-time").html(response.data);
                }
            },
            error: function (xhr, status, err) {
            }
    ...
    $('#piechartOverview_graph_data .highcharts-legend-item tspan').click(function () {
        ...
        var total_week_rec = $(".overview-graph-tbl").find("tbody").find("tr:not(.hide-tbl-record)").length;
        $(".overall-event_total").html("<strong>"+total_week_rec+" Events</strong>");
        

        var display_type ="";
        if($(".overview-graph-tbl table .Typio-OL").length > 0){  
        ...
        if($(".overview-graph-tbl table .Propack").length > 0){
         ...
         if($(".overview-graph-tbl table .Quick-Cards-OL").length > 0){  
         ...
         if($(".overview-graph-tbl table .Arcade-OL").length > 0){ 
...
$("#new-option-btn-edit-deck").click(function () {
        $("#new-option-section-wrap-edit-deck").toggle();
        $(this).toggleClass('import-menu-display');
 });
$("#new-option-btn-edit-test").click(function () {
        $("#new-option-section-wrap-edit-test").toggle();
        $(this).toggleClass('import-menu-display');
 });
