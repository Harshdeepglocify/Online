...
$(".new-option-btn").click(function () {
    ...
    obj.find(".new-option-section-wrap").toggle();        
});
...
//            message: "Are you sure you want to remove <b>" + first_name + "</b> from your dashboard ?",
 ...
 $(document).on('click', '.create_clk_link', function () {
  
     var this_data = $(this);
console.log( this_data );
inputID = $(this).attr('id');
inputID  = inputID.substring(4);
// $('.bootbox-confirm').hide();
//   $('.bootbox-close-button').click();
//    $('#' + inputID).parent().hide();
console.log('inputID ' + inputID );
var exp_Val = $('#num'+inputID).val();
console.log(exp_Val);
var exp_per = $('#intv'+inputID).val();
console.log(exp_per);
var wait1 = $('#wt_'+inputID).val();
console.log($('#wt_'+inputID).val());
console.log('wait'+ wait1 );

var user_id = inputID.substring(4);  
  $('#' + inputID).parent().show();
        var input = document.getElementById(inputID);
        var input_value =$("#"+inputID).val();
console.log('input_Va; ' + input_value );
        var isiOSDevice = navigator.userAgent.match(/ipad|iphone/i);
        
$.ajax({
         type: 'POST',
         url: ADMIN_URL + 'config/config-student.php',
         data: {'user_id': user_id , 'action': 'Generate_login_link', 'exp_Val':exp_Val,'exp_per':exp_per , 'wait': wait1 ,'inputID':inputID },
         async: true,
         cache: false,
         timeout: 10000,
         success: function (response) {
...
//console.log(response );
        if (response.status == '200') {
            $('#newtext').html(response.url);
$('#admin_student_list_table').DataTable().ajax.reload(null, false);
$('#student_list_table').DataTable().ajax.reload(null, false);
...
//         bootbox.confirm({ 
//              title: "<span style='font-size: 28px; font-weight: bold;'>Delete Student",
...
message: "<span id='newtext'><p style='font-size:18px'>This link will log your student in without needing to type in their username and password. Be careful! Anyone can log in to the student's account using this link. Treat it like a password and share it carefully.</p><p style='font-size:18px'>The link will no longer be vaild once it expires, a new link is created, or if the student's password is changed.</p><p style='font-size:18px'>You can optionally include a wait page that requires the user to click a single button before they are logged in. This is useful if creating a home screen shortcut on IOS devices.</p><div class='col-md-12'> <div class='col-md-6'><h4 style='font-weight:bold'>Link Expiration</h4></div><div class='col-md-6'><h4 style='font-weight:bold'>Wait Page</h4></div></div>"+new_link+"</span>",
 ...
 buttons: {
    ...
    $(document).on('click', '.one-click-login-old', function () {
        var this_data = $(this);
        var first_name = this_data.attr('data-name');
    ...
    $(document).on('keypress', '.delete_multi_student_text', function (e) {    
        if(e.keyCode == 13){
            e.preventDefault();
            jQuery(".delete_multi_student_btn").click();
        }
    });
	
	//$(document).on('change', '#report_filter_dropdown', function() {
	$(document).on('click', '#report_delete_btn', function () {
		//alert("Selected: " + $(this).val());
		//var selectedVal = $(this).val();
		//if('delete' == selectedVal){
		 var reports_to_del = [];
                var str = '';
                $("input[name='report_all_delete[]']:checked").each(function () {
                    reports_to_del.push($(this).data('id'));                 
                });
				if (reports_to_del.length == 0) {
                    alert('Please select a Report first!');
                } else {
				 $.ajax({
                type: 'POST',
                url: ADMIN_URL + 'config/config-student.php',
                data: {'action':'ajax_delete_report_data','is_ajax' : '1','report_id':reports_to_del},
                async: true,
                cache: false,
				beforeSend: function () {
                    $('#spinner_loader').show();                
                ...
                success: function (response) {
                    ...
                    if (response.status == true) {
					   ...
					   alert(response.msg);
					    $('#spinner_loader').hide();
						$('#student_download_list_table').DataTable().ajax.reload(null, false);
					//	$('#report_filter_dropdown').val('');
				   }else{
	...
	/*05SEPT2025*/
	$(document).on('click', '.delete_reportbutton_btn', function () {
        var this_data = $(this);
        var text_data = $('#delete_text_report').val();
        var delete_id = $('#delete-report-id').val();
        if (text_data == "") {
            $('#delete_text_report').focus();
            return false;
        }
        ...
        if (text_data.toLowerCase() == 'delete') {
            $.ajax({
                type: 'POST',
                url: ADMIN_URL + 'config/config-student.php',
                data: {'report_delete_id': delete_id, 'action': 'delete_report_user'},
                async: true,
                cache: false,
                timeout: 10000,
				beforeSend: function () {
	...
	$(document).on('click', '#report_student_icon_id', function () {
      // alert('Student Alert');
		var this_data = $(this);
		let student_arr = $(this).data('studentsid'); 
		
		$.ajax({
            type: 'POST',
            url: ADMIN_URL + 'config/config-student.php',
            data: {'is_ajax':1,'students_id':student_arr, 'action': 'ajax_get_student_data'},
            async: true,
            cache: false,
            timeout: 10000,
			beforeSend: function () {
            ...
            success: function (response) {
				...
				console.warn(response);
				 $('#student-list-file tbody').html(response.html);
                 $('#student-list-file').modal('show');
		...
		$(document).on('click', ' a.accessibyte-link.cancel_req', function () {
            ...
            type: 'POST',
            url: ADMIN_URL + 'config/config-student.php',
            data: {'report_cancel_id':fileId, 'action': 'cancel_report_user'},
            async: true,
            cache: false,
            timeout: 10000,
			beforeSend: function () {
...
/* START : Student Data CSV Download Code ON 03-09-2021 ADDED BY PHP DEV 3 */
    $(document).on('click', '.student-export-to-csv', function () {
        var file_val= $(this).attr('data-id');
		console.warn(file_val);
        if(file_val == 'student-admin-list'){
          // $file_name = 'student/export_student_data.php'; 
            $file_name = 'student/export_add_queue.php'; 
        }else{
              //$file_name = 'student/export_student_datasingle-teacher.php';
              $file_name = 'student/export_add_queue_single.php';
        }
        $.ajax({
            type: 'POST',
            url: ADMIN_URL + $file_name,
            data: '',
            async: true,
            beforeSend: function () {
                $('.spinner_loader').show();                
            ...
            // or show on page
				...
				//console.log(data.Success);
				 //$('.student-add-column-cst-msg h4').text('hello');
             /*
             ...
             */
              ...
              downloadLink.download = "Accessibyte Student List.csv";
              /*
    ...
    /* END : Student Data CSV Download Code ON 03-09-2021 ADDED BY PHP DEV 3 */
    /* START : Student Data CSV Download Code ON 03-09-2021 ADDED BY PHP DEV 3 */
   /* $(document).on('click', '.student-export-to-csv', function () {
        var file_val= $(this).attr('data-id');
        if(file_val == 'student-admin-list'){
...
function copyClipBoard(claName){
    var $input = $('.'+claName);
    //$input.val(result);
