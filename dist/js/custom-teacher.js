$(document).ready(function () {
    ...
    jQuery(document).on('keypress', '.delete_multi_teacher_text', function (e) {    
        if(e.keyCode == 13){
            e.preventDefault();
            jQuery(".delete_multi_teacher_btn").click();
        }
    });
    $(document).on('click', '.delete_multi_teacher_btn', function () {
        ...
        if (text_data.toLowerCase() == 'delete') {
            $.ajax({
                ...
                data: {'teacher_delete_id': teacher_id, 'action': 'delete_multi_teacher_account'},
                async: true,
                cache: false,
                timeout: 10000,
    ...
    $(document).on('click', '.update_student_limit', function () {
        ...
        $.ajax({
            type: 'POST',
            url: ADMIN_URL + 'config/config-teacher.php',
            data: {'teacher_id': teacher_id,'student_list_limit':student_list_limit, 'action': 'update_teacher_student_limit'},
            async: true,
            cache: false,
            timeout: 10000,
            success: function (response) {
                ...
                if (response.status == true) {
                    ...
                    $('.alert-msg-response').find('.alert').removeClass('alert-warning alert-success').html(response.msg + " " + response.error).show();
                } else {
                    $('.alert-msg-response').find('.alert').removeClass('alert-warning alert-success').html(response.msg + " " + response.error).show();
                }                    
            }
     ...
     /* START : Teacher Data CSV Download Code ON 03-09-2021 ADDED BY PHP DEV 3 */
    $(document).on('click', '.teacher-export-to-csv', function () {
        $.ajax({
            type: 'POST',
            url: ADMIN_URL + 'teacher/export_teacher_data.php',
            data: '',
            async: true,
            beforeSend: function () {
                $('.spinner_loader').show();                
             ...
             */
              ...
              downloadLink.download = "Accessibyte Teachers List.csv";
              /*
