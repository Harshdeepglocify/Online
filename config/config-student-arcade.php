    ...
    if (!empty($_POST['form_data']) && !empty($_POST['type']) && $_POST['type'] == 'Add-Hangman') {
        ...
        if (!empty($re_data['title'])) {
            ...
            foreach($form_data['fields'] as $key => $val){
                $fieldsArr[] = addslashes($val);
            }
            $data = implode('`',$fieldsArr);
    ...
    if (!empty($_POST['form_data']) && !empty($_POST['type']) && $_POST['type'] == 'Update-Hangman') {
        ...
        if (!empty($form_data['fields'])) {
            if (!empty($form_data['table_id'])) {
                ...
                if ($user_type == 1) {
                    $result['html'] .= '<td><input type="checkbox" name="share_hangman_ids[]" value="' . $form_data['table_id'] . '" class="share_hangman_ids"></td>';
                }    
                $result['html'] .= '<td>' . $form_data['title'] . '</td>';
...
function updateAppArcadeSettingData($update_arcade_setting_data = '') {
    ...
    if(isset($_POST['ArcadeSettingSubmit']) && $_POST['ArcadeSettingSubmit'] == 'Save Arcade Settings For All Students'){
        $args = array(
            'user_id' => $studentid,
            'role' => 'student',
        );
    ...
    foreach ($update_arcade_setting_ready_data as $update_arcade_setting_data_id => $update_arcade_setting_data_variable) {
        if(isset($student_data[0]) && !empty($student_data[0])){
            ...
            foreach($student_all_data as $sval){    
                $update_arcade_setting_status = updateAppArcadeSettingField($update_arcade_setting_data_id, $update_arcade_setting_data_variable,$sval['id']);
            }    
        } else {
            $update_arcade_setting_status = updateAppArcadeSettingField($update_arcade_setting_data_id, $update_arcade_setting_data_variable,$studentid);
        }
 ...
 * @return boolean Arcade-OL table update status
 */
function updateAppArcadeSettingField($update_arcade_setting_item_id = 0, $update_arcade_setting_variable = '',$id = 0) {
    global $con, $studentid;
    $arcade_settings = 'settings';
    //Check if setting already exists for student
    $query = query("SELECT COUNT(*) as total FROM " . $arcade_settings . " WHERE id=" . $id . " AND item='" . $update_arcade_setting_item_id . "'");

    $data = fetch($query);
    if ($data['total'] != 0) {
        $typio_settings_update_query = "UPDATE `" . $arcade_settings . "` SET `variable`='" . $update_arcade_setting_variable . "' WHERE `id`='" . $id . "' AND `item`=" . $update_arcade_setting_item_id;
        $typio_settings_update_query_run = mysqli_query($con, $typio_settings_update_query);

        return $typio_settings_update_query_run;
    } else {
        $typio_settings_insert_query = "INSERT INTO " . $arcade_settings . " (`id`,`item`,`variable`) VALUES( " . $id . ",'" . $update_arcade_setting_item_id . "', '" . $update_arcade_setting_variable . "' )";
