...
function Typio_average_chart_write($student_id = 0, $typio_start_date = 0, $typio_end_date = 0) {
    ...
    $_average_scale['Combo'] = !empty($log_data_table_combo_value) ? floor(($log_data_table_combo_value / $log_typio_data_count)) : '0' ;
...
function displayAppTypioData($typio_start_date = 0, $typio_end_date = 0, $student_id = '') {
    ...
    $table_data = array();
    $studentid = !empty($student_id) ? $student_id : $studentid;
    $log_typio_query = "SELECT * FROM `log` WHERE `app` LIKE 'Typio-OL' AND id ='" . $studentid . "'";
    ...
    $log_typio_data_html = '<tbody><tr><th>Lesson</th><th>Date</th><th>WPM</th><th>ACC</th><th>COM</th><th></th></tr>';
    ...
    while ($log_typio_data_row = mysqli_fetch_assoc($log_typio_data)) {
        ...
        $log_typio_data_html .= '</td><td>';
        $log_typio_data_html .= ($log_typio_data_value[2]) ? $log_typio_data_value[2] : '';
...
function displayAppTypioDataTableAverageBarChart($student_id = 0, $typio_start_date = 0, $typio_end_date = 0) {
    ...
    $log_typio_data_html .= '<tr><td>' . $log_data_table_combo_title . '</td><td>' . (!empty($log_data_table_combo_value) ? floor(($log_data_table_combo_value / $log_typio_data_count)) : '0' ) . '</td></tr>';
...
function displayAppTypioDataTableAverageAreaChart($typio_start_date = 0, $typio_end_date = 0, $student_id = '') {
    ...
    $log_typio_data_html = '<tbody><tr><th>Name</th><th> WPM</th><th>% Accuracy</th></tr>';
    ...
    if ($log_typio_data) {
        ...
        $log_typio_data_html .= '<tr><td>0</td><td>0</td><td>0</td></tr>';
    }

    if ($log_typio_data_html == '<tbody><tr><th>Name</th><th>WPM</th><th>ACC(%)</th></tr>') {
        $log_typio_data_html .= '<tr><td></td><td></td><td></td></tr>';
        return $log_typio_data_html;
    }
...
if (isset($_POST['share_ids']) && isset($_POST['share_user_list']) && !empty($_POST['type'])) {
    ...
    if (!empty($share_ids)) {
        ...
        $result['status'] = 1;
        $result['msg'] = '<h4><i class="icon fa fa-check"></i> Selected ' . $type . ' shared successfully. <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a></h4></h4>';
    }
