...
define('STUDENT_PANEL', WEB_PATH . 'apps/typiodir/');
...
function AddUserDefault_ExtaText($user_id) {
    ...
    $query = query("INSERT INTO text (id, app, number, title, data) VALUES 
                            ...
                            ( '" . $user_id . "', 'Arcade-OL', '2', 'Winter', 'Snow`Ice`Gloves`Cold`Coat`Freeze`Chill`Snowman`Hot Cocoa'),
            ...
            if (!empty($result)) {
                ...
                if (isset($_POST['update_license_withoutLogin']) && $_POST['update_license_withoutLogin'] == 'withoutlogin') {
                    if (!empty($result) && $result['role'] == 'teacher') {
                        ...
                        if ($new_update_license_limit < $studentsCount) {
                            $diffoflimit = $studentsCount - $new_update_license_limit;
                            //$_SESSION['error']['message'] = "Not enough seats on this license. You have " . $studentsCount . " students and this license has " . $new_update_license_limit . " seats. Please delete " . $diffoflimit . " students before applying the license.";
							$_SESSION['error']['message'] = "Not enough seats on this license. You have have " . $studentsCount . " seats available. Please delete " . $diffoflimit . " students before applying the license.";
                            $flag = 0;
                        }
                    }
                }
