...
if (isset($_POST) && !empty($_POST)) {
    ...
    if (isset($_POST['Submit']) && $_POST['Submit'] == 'Update License') {
        ...
        if (empty($username)) {
            $error = 1;
            $_SESSION['error']['message'] = 'User Name can not leave empty';
            $_SESSION['error']['color'] = 'danger';
        }
        if (empty($password)) {
            $error = 1;
            $_SESSION['error']['message'] = 'Password can not leave empty';
            $_SESSION['error']['color'] = 'danger';
        }
        if (empty($license)) {
            $error = 1;
		...
		if(!isset($_POST['agree']) && empty($_POST['agree'])){
			$error = 1;
            $_SESSION['error']['message'] = 'Please agree terms and privacy policy.';
            $_SESSION['error']['color'] = 'danger';
		}
        ...
        if ($error == 0) {
            ...
            update_license($username, $password, trim($license));
        }
    }
}
        ...
        <div class="container">
            ...
            if (isset($_SESSION['error']) && !empty($_SESSION['error'])) {
                ...
                <div style="font-family: Roboto Slab, serif; background-color: #000; color: #FFFFFF" class="alert  alert-dismissable fade in">
                    ...
                    if($_SESSION['error']['message'] == 'License is updated successfully.'){
                        $_POST['username'] = '';
                        $_POST['password'] = '';
                        $_POST['license'] = '';
                    }
            ...
            <div class="login-box-body">
                <div class="register_header">
                    ...
                    <div class="col-xs-12 form-group input_box no-padding">
                        <p>This form allows you to update your current Accessibyte Online license. Doing so will replace your existing or expired license with the new one. If you don't have a license to active, you'll need to purchase one before filling out this form.</p>
                    </div>
                </div>    
                    ...
                    <form action="<?php echo LICENSE_UPDATE_FILE_PATH ?>" method="POST" id="update_license_frm" class="margin-bottom">
						...
						<div class="form-group text-center">
                            ...
                            <div class="registerCheck form-group">
								...
								<label for="agree"> I agree to Accessibyte's <a href="<?php echo WEB_PATH; ?>terms-privacy" target="_blank" class="accessibyte-link"> terms and privacy </a>
								</label>
							</div>
                            
						</div>
                        <div class="form-group">
                            <input type="submit" value="Update License" name="Submit" class="btn btn-info pull-right  col-md-12" />
                        </div>
                    </form>
                </div>
                <br>
